resource "aws_eks_node_group" "nodegroup" {
  ...
  version         = local.use_custom_ami ? null : (var.eks_node_group_info.kubelet_version == "" ? var.eks_cluster_version : var.eks_node_group_info.kubelet_version)
  ...
  dynamic "taint" {
    for_each = [for taint in var.eks_node_group_info.taints : taint if try(taint.key, "") != ""]
